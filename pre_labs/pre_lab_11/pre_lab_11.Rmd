...
**Answer**
A bunch of files with the same names but different endings
```{r, echo=FALSE}
...
**Answer**
33 columns with naics codes, latitiude and longitude, beds, and geometry
```{r}
...
**Answer**
a sampling of 10 hospitals and their information
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
50 are general acute care, which is the majority, the next is 11 psychiatric
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
information about general acute care hospitals in maryland
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
scatterplot, looks like the points are at latitude and longitude
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
a bunch of information, including geometry
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe with only the information from maryland
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Maryland with all of the counties outlined
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Points are now on top of the map, we can see that they are concentrated around a county
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dots are different colors showing the number of beds
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
maryland general acute care hospitals again
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
maryland population with geometry
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Cleaned up county names, population, and geometry
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
beds joined into the previous data
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
replaced the N/A with a 0 value
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
new column with beds per 100k people
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
colorful map with counties filled in based on beds per 100k
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
color differences are more obvious
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
colors are even more different, and labeled
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
