...
**Answer**
Dataframe with all five fields for the states
```{r}
...
**Answer**
a scatter plot also showing a trend line and shadow, indicating that many businesses that received ppe were owned by Trump supporters
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
a similar plot, but the trends vs. the dots look less clear because they seem to be farther from the line
```{r}
...
**Answer**
N/A for pct_trump, 0.52% for businesses_receiving_ppe_pct, 0.22% for ppe_amount_per_employee
```{r}
reveal_data %>%
...
**Answer**
0.52 correlation, which isn't a super meaningful result, though it is a stronger correlation than the previous
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
the correlation is only 0.221133 and the p-value is 0.1189, so the correlation is very low, but the chance of error is much higher, so the result is not trustworthy
```{r}
...
**Answer**
Trump voters had higher mean ppp amount per employee and mean percent of businesses receiving ppe 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
the p-value is 0.2295, which is very high, so the results are not relaible
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
the p-value is 0.002643, which is low, so the results are probably more trustworthy
```{r}
