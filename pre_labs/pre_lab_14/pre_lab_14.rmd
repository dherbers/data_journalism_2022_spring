...
**Answer**
Only shows the words that are unique, does not show repeats of the exact same words.
```{r}
...
**Answer**
A column of words with a row for each unique word found. It also includes numbers.
```{r}
...
**Answer**
It shows the words most often used, I do not think that it is interesting because it mostly includes prepositions.
...
**Answer**
The words are more interesting, but there are still multiple iterations of the same word, COVID19, which is only really interesting if you are looking at the ways that people refer to the illness.
```{r}
unique_words %>%
...
**Answer**
The words are very similar, but 2021 also features vaccines.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Two word phrases, including health care, public health, and town hall
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Most of the two word phrases are about vaccines.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
There are fewer republican tweets, and democratic tweets focus more on health care.
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
President Trump was featured very heavily in 2020, along with god bless. Illegal immigrants entered in 2021.
```{r}
covid_tweets %>%
...
**Answer**
The sentiments all seem pretty correct and intuitive.
```{r}
