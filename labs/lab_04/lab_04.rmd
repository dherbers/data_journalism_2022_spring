...
```{r}
maryland_ppp_naics <- maryland_ppp %>% 
...
```{r}
maryland_ppp_naics_zcta <- maryland_ppp_naics %>% 
...
**A4** zip: 20850, percentage: 23.5%, count: 240
...
**A5** The race category in the original data was not collected well, so there are probably more Asian applicants whose race is listed as unknown because they did not answer or because there are multiple owners of different races.
...
**A6** All three zipcodes (21031, 20701, 21240) are in areas with extrememly small populations, one of them is actually an airport, so the business owners probably do not live in the same zipcode that their business exists in, and the ZCTA data goes off of where populations actually live.
...
**A7** 44 industries had exactly one loan. The categories that I would want to look into are credit card issuing and savings institutions because the other categories are specific kinds of manufacturing that would make sense for single small businesses, but those two categories do not make sense as small businesses. It seems like there would be more credit card issuing and savings institutions that needed loans if they had a legit reason to apply.
