...
**A1.**
126 E Burke Street had 6 loans, to the businesses DREW HOLDINGS LLC, BRIX27, LLC, ABRAHAM ASHTON, HUB CO-OP LLC, HUB CO-OP, LLC, and 	RONIN PROPERTIES LLC

```{r}
zip_25401_clean <- read_csv("data/zip_25401_loan_clean.csv")
...
**A2.**
The street address is the location of an office space rental agency called the Hub workspace, so it would not be immediately suspicious. Drew Holdings LLC and Abraham Ashton are suspicious though, because Ashton is listed as one of the directors of Drew Holdings, so he may be double dipping. Brix 27 is directed by Ashton also. Ashton is listed as the director of Hub Coop and Ronin Properties also, which is very suspicious. Both Hub Coop varieties must be the same company because there are no other variants at the address listed on Open Corporate. Robert Johnson is also connected to all of the companies, mostly listed as an agent.
...
wv_berkeley_loans <- wv_ppp %>%
  filter(project_county_name == "BERKELEY")

write_csv(wv_berkeley_loans, "data/wv_berkeley_loans.csv")
...
wv_berkeley_naics <- wv_berkeley_clean %>% 
...
Guidance: there are a lot of steps you'll need to take to answer this question. You may find it helpful to write out in English what you plan to do step-by-step before you start writing code.   # write_csv accepts 2 arguments: the dataframe and the path to the file you write out
write_csv(wv_population_county, "data/wv_population_county.csv")

**A3.**
Martinsburg had the most loans in Berkeley county with 1,166. Full Service Restaurants had the most loans in Martinsburg with 50 loans.
...
**A4.**
I would guess that Full Service Restaurants needed more loans because of restrictions on indoor dining due to the COVID-19 pandemic, so that could be reported on further my checking which mandates were in place at that time. Another reason could be the density of restaurants in the city compared to other businesses.
...
```{r}
wv_martinsburg_restaurants <- wv_berkeley_naics %>%
...
**A5.**
The name of the business that got the most loans was Cottle Christi L LLC, and it got a loan with the amount $280,434 that was approved on February 17, 2021. The business listed 94 jobs retained. The business is known by customers as Kitzie's Cafe or Kitzie's Restaurant and Lounge, not Cottle Christi L. There are two locations in Martinsburg and Spring Mills. There used to be a location in Inwood, but I did not find any information about when it closed. The last reviews on Google and Yelp are from two years ago, and I did not see anything when I searched on Facebook.
